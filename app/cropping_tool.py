...
ALL_TARGETS = obs_crop_service.ALL_TARGETS
...
class CropTool(tk.Tk):
    def __init__(self):
        ...
        self.bind("<FocusIn>", self.on_focus_refresh_race)
    ...
    def _build_ui(self):
        ...
        ttk.Button(topbar, text="Open Screenshot", command=self.open_image).pack(side=tk.LEFT)
        ttk.Button(topbar, text="Take/Load Slot Screenshot", command=self.capture_and_load_selected_slot).pack(side=tk.LEFT, padx=(6,0))
        ...
        conn = ttk.LabelFrame(right, text="OBS Connection")
        conn.pack(fill=tk.X, padx=8, pady=8)
        row = ttk.Frame(conn); row.pack(fill=tk.X, padx=6, pady=2)
        ...
        row = ttk.Frame(conn); row.pack(fill=tk.X, padx=6, pady=4)
        ...
        self.crop_var = tk.StringVar(value="Left -   Right -   Top -   Bottom -")
        ttk.Label(results, textvariable=self.crop_var, justify=tk.LEFT).pack(anchor="w", padx=6, pady=6)
        ...
        runner_memory = ttk.LabelFrame(right, text="Runner Crop Memory")
        ...
        for part in ["Stream", "Tracker", "Timer"]:
            ...
            ttk.Label(row, text=part, width=8).pack(side=tk.LEFT)
            ttk.Button(row, text="Load", command=lambda p=part: self.load_runner_part_crop(p)).pack(side=tk.LEFT, padx=(0, 5))
            ttk.Button(row, text="Save/Apply", command=lambda p=part: self.save_apply_runner_part_crop(p)).pack(side=tk.LEFT, padx=(0, 5))
            var = tk.StringVar(value=f"{part}: no current runner")
            self.part_status_vars[part] = var
            ttk.Label(row, textvariable=var).pack(side=tk.LEFT)

        action_row = ttk.Frame(runner_memory)
        action_row.pack(fill=tk.X, padx=6, pady=(6, 6))
        ttk.Button(action_row, text="Apply All Saved", command=self.apply_all_runner_crops).pack(side=tk.LEFT)
        ttk.Button(action_row, text="Refresh Race", command=self.refresh_current_race).pack(side=tk.LEFT, padx=(6, 0))

        self.target_var = tk.StringVar(value=ALL_TARGETS[0])
    ...
    def open_image(self):
        path = filedialog.askopenfilename(title="Open screenshot", filetypes=[("Images", "*.png;*.jpg;*.jpeg;*.bmp;*.webp"), ("All files", "*.*")])
        if not path:
            return
    ...
    def load_image_file(self, path):
        try:
            ...
            self.crop = None
            self.crop_var.set("Left -   Right -   Top -   Bottom -")
            self.redraw_image()
        except Exception as e:
    ...
    def clear_screenshot_folder(self):
        ...
        files = [
            ...
            if os.path.isfile(os.path.join(folder, f)) and os.path.splitext(f)[1].lower() in {".png", ".jpg", ".jpeg", ".bmp", ".webp"}
        ]
        if not files:
    ...
    def update_part_statuses(self):
        ...
        if not runner:
            for part, var in self.part_status_vars.items():
                var.set(f"{part}: no current runner")
            return
    ...
    def target_details(self, source_name):
        match = re.match(r"^(?P<mode>[24]P)\s+R(?P<slot>[1-4])\s+(?P<part>Stream|Tracker|Timer)$", source_name, re.I)
        if not match:
            return None
        return {
